...
const colorPalette = ['#9D6FA8', '#c24f4f', '#4f7ec2', '#4cA87a', '#c9874c', '#BFA05A', '#A85A8B'];
      ...
      <div className={styles.gridRow}>
        ...
        <div className={styles.card}>
          ...
          <div className={styles.romanceBars}>
            {loveInterests.map((name, idx) => {
              const total = stats.romanceTotals[name] || 0
              const max = Math.max(...loveInterests.map(n => stats.romanceTotals[n] || 0), 1)
              const pct = Math.round((total / max) * 100)
              const color = getColor(idx + 1) // Offset color slightly from POV array
              return (
                <div key={name} className={styles.povRow}>
                  <span className={styles.povName} style={{ color }}>{name}</span>
                  <div className={styles.povBarWrap}>
                    <div className={styles.povBarFill} style={{ width: `${pct}%`, background: color }} />
                  </div>
            ...
            <LineChart data={romanceData} margin={{ top: 6, right: 12, left: -20, bottom: 0 }}>
              ...
              {loveInterests.map((name, idx) => (
                <Line key={name} type="monotone" dataKey={name}
                  stroke={getColor(idx + 1)} strokeWidth={2} dot={false} />
              ))}
            </LineChart>
          </ResponsiveContainer>
        </div>
          ...
          {chapters.map(ch => {
            ...
            return (
              ...
              >
                ...
                <span className={styles.chPov} style={{ color: pov ? getColor(povCharacters.indexOf(pov) > -1 ? povCharacters.indexOf(pov) : 0) : 'var(--ivory-muted)' }}>
                  {pov || '—'}
                </span>
